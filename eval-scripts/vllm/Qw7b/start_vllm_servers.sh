#!/bin/bash
# Start 4 vLLM servers, one per GPU
...
echo ""
echo "=== Starting vLLM servers on 4 GPUs ==="

CUDA_VISIBLE_DEVICES=0 vllm serve Qwen/Qwen2.5-VL-7B-Instruct --host 0.0.0.0 --port 8000 --dtype bfloat16 --max-model-len 32768 > /tmp/vllm_0.log 2>&1 &
echo "Started server on GPU 0, port 8000"

CUDA_VISIBLE_DEVICES=1 vllm serve Qwen/Qwen2.5-VL-7B-Instruct --host 0.0.0.0 --port 8001 --dtype bfloat16 --max-model-len 32768 > /tmp/vllm_1.log 2>&1 &
echo "Started server on GPU 1, port 8001"

CUDA_VISIBLE_DEVICES=2 vllm serve Qwen/Qwen2.5-VL-7B-Instruct --host 0.0.0.0 --port 8002 --dtype bfloat16 --max-model-len 32768 > /tmp/vllm_2.log 2>&1 &
echo "Started server on GPU 2, port 8002"

CUDA_VISIBLE_DEVICES=3 vllm serve Qwen/Qwen2.5-VL-7B-Instruct --host 0.0.0.0 --port 8003 --dtype bfloat16 --max-model-len 32768 > /tmp/vllm_3.log 2>&1 &
