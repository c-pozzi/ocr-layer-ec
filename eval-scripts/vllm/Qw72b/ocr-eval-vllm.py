"""
OCR Evaluation with vLLM - Qwen2.5-VL-72B version

This script uses Qwen2.5-VL-72B with tensor parallelism (4 GPUs per server, 2 servers)
for comparing PDF OCR layer vs AI Model OCR vs Ground Truth.

Usage:
1. Start vLLM servers with tensor parallelism:
   bash start_vllm_72b.sh

2. Run this script:
   python ocr_eval_vllm_72b.py /path/to/ocr-evaluation-samples/
"""
...
# ============================================================================
# CONFIGURATION FOR 72B MODEL WITH TENSOR PARALLELISM
# ============================================================================
# 2 servers, each using 4 GPUs with tensor parallelism
VLLM_URLS = [
...
async def process_single_image(
    ...
    async with semaphore:
        ...
        payload = {
            "model": MODEL_NAME,  # Uses 72B model
            "messages": [
                {
                    "role": "user",
                    "content": [
...
async def process_all_documents(pairs: list, dpi: int = 150) -> tuple:
    ...
    print(f"Using {len(VLLM_URLS)} servers with {MODEL_NAME}")
    ...
    for image_path, gt_path, doc_id in pairs:
        ...
        print(f"  {doc_id}: AI Model CER: {qwen_cer:.2%}", end="")
        if pdf_ocr_cer is not None:
            diff = pdf_ocr_cer - qwen_cer
            better = "✓ AI Model better" if diff > 0 else ("✗ PDF better" if diff < 0 else "= Same")
            print(f" | PDF CoDe CER: {pdf_ocr_cer:.2%} | {better}")
        else:
            print()
...
def main(eval_dir: str, dpi: int = 150):
    ...
    print(f"Servers: {len(VLLM_URLS)} (tensor parallelism)")
    ...
    if qwen_cer_values:
        print(f"{'AI Model (72B)':<20} {statistics.mean(qwen_cer_values):>9.2%} {statistics.median(qwen_cer_values):>9.2%} {min(qwen_cer_values):>9.2%} {max(qwen_cer_values):>9.2%}")
    
    if pdf_cer_values:
        print(f"{'PDF CoDe':<20} {statistics.mean(pdf_cer_values):>9.2%} {statistics.median(pdf_cer_values):>9.2%} {min(pdf_cer_values):>9.2%} {max(pdf_cer_values):>9.2%}")
    ...
    if qwen_cer_values and pdf_cer_values:
        ...
        print(f"{'='*70}")
        print(f"AI Model better: {qwen_better:>3} documents")
        print(f"PDF CoDe better: {pdf_better:>3} documents")
        print(f"Same:            {same:>3} documents")
        print(f"Avg improvement: {avg_improvement:>+.2%} (positive = AI Model better)")
        print(f"{'='*70}")
    
    # Save detailed results - use OUTPUT_DIR for 72B results
    ...
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["doc_id", "num_pages", "ai_model_72b_cer", "pdf_ocr_cer", "improvement", "inference_time_sec", "error"])
        for r in results:
            improvement = ""
    ...
    for r in successful:
        # Save AI Model OCR (raw)
        qwen_path = ocr_output_dir / f"{r['doc_id']}_ai_model_72b_ocr.txt"
        with open(qwen_path, "w", encoding="utf-8") as f:
            f.write(r["qwen_ocr_text"])
        
        # Save AI Model OCR (normalized)
        qwen_norm_path = ocr_output_dir / f"{r['doc_id']}_ai_model_72b_ocr_norm.txt"
    ...
    with open(summary_path, "w", encoding="utf-8") as f:
        f.write(f"OCR Evaluation Summary - Qwen2.5-VL-72B\n")
        f.write(f"{'='*70}\n")
        ...
        if qwen_cer_values and pdf_cer_values:
            ...
            f.write(f"  AI Model better: {qwen_better} documents\n")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python ocr_eval_vllm_72b.py <eval_directory> [dpi]")
        print("\nExpected directory structure:")
        print("  <doc_id>.pdf or <doc_id>.tif")
        print("  <doc_id>_ground_truth.txt")
        print("\nMake sure vLLM servers are running first (start_vllm_72b.sh)!")
        print(f"\nModel: {MODEL_NAME}")
        print(f"Servers: {len(VLLM_URLS)} with tensor parallelism")
        sys.exit(1)
