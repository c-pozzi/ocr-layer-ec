...
Pre-renders a page sample to base64 and caches in memory for consistent benchmarking.
"""
...
# Add parent dir for poc imports
...
def discover_pages(input_dir: Path, max_pages: int = SAMPLE_SIZE) -> list[dict]:
    ...
    Randomly samples max_pages from the total.
    """
...
def pre_render_pages(pages: list[dict], dpi: int = DPI) -> list[dict]:
    """
    Pre-render all pages to base64. Stores image_b64 in each page dict.
    Returns only successfully rendered pages.
    """
...
async def run_ocr_workload(
    ...
    servers: list[str],
    config: BenchmarkConfig,
    workload_type: str,  # "simple_ocr" or "complex_ocr"
) -> list[dict]:
    ...
    if workload_type == "simple_ocr":
        ...
        # For complex, use a representative classification for prompt building
...
async def run_warmup(servers: list[str], config: BenchmarkConfig, sample_page: dict):
    """Send warmup requests to fill CUDA graphs / KV cache."""
...
async def run_single_config(
    ...
    """Run all workloads for a single config (start servers, benchmark, stop)."""
    print(f"\n{'='*70}")
    print(f"CONFIG: {config.config_id}")
    print(f"  Model: {config.model}")
    print(f"  TP={config.tp}, Servers={config.num_servers}, "
          f"CUDA graphs={'on' if config.cuda_graphs else 'off'}")
    print(f"  gpu_mem={config.gpu_mem_util}, max_seqs={config.max_num_seqs}, "
          f"max_model_len={config.max_model_len}")
    if config.max_num_batched_tokens:
        print(f"  max_batched_tokens={config.max_num_batched_tokens}")
    print(f"{'='*70}")
    ...
    try:
        ...
        healthy_servers = await wait_for_healthy(config, timeout=300)
        print(f"  {len(healthy_servers)} servers ready")

        # Sample pages for this config's workloads
        ...
        # Snapshot GPU memory after warmup
        ...
        # Brief pause for GPU memory to free
...
async def run_benchmark(args):
    """Main entry point: sample pages, loop configs, run workloads."""
    ...
    print(f"Benchmark: {len(configs)} configs to test")
    print(f"Instance: p4d.24xlarge (8x A100 80GB)")

    # Discover and sample pages
    ...
    # Shuffle for variety across configs
    random.seed(42)
...
def parse_args():
    parser = argparse.ArgumentParser(description="A100 vLLM Benchmark Runner")
    parser.add_argument(
